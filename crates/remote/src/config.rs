...
impl AuthConfig {
    fn from_env() -> Result<Self, ConfigError> {
        let jwt_secret = env::var("CREW_REMOTE_JWT_SECRET")
            .map_err(|_| ConfigError::MissingVar("CREW_REMOTE_JWT_SECRET"))?;
...
fn validate_jwt_secret(secret: &str) -> Result<(), ConfigError> {
    let decoded = BASE64_STANDARD
        ...
        .map_err(|_| ConfigError::InvalidVar("CREW_REMOTE_JWT_SECRET"))?;

    if decoded.len() < 32 {
        return Err(ConfigError::InvalidVar("CREW_REMOTE_JWT_SECRET"));
    }
