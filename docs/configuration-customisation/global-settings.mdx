...
<Frame>
<img src="/images/vk-settings.png" alt="Crew global settings page showing theme options, agent configuration, and settings" />
</Frame>
...
<Frame>
<img src="/images/vk-editor-ssh.png" alt="Crew settings editor section showing ssh configuration options." />
</Frame>

When running Crew on a remote server (e.g., accessed via Cloudflare tunnel, ngrok, or as a systemctl service), you can configure VSCode-based editors to open projects via SSH instead of assuming localhost.
...
Set a prefix for auto-generated branch names (e.g., `crew` results in `crew/task-name`). Leave empty for no prefix.
...
Enable or disable telemetry data collection to help improve Crew.
