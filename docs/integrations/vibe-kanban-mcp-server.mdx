---
title: "Crew MCP Server"
description: "Configure the Crew MCP server"
---

Crew exposes a local MCP (Model Context Protocol) server, allowing you to manage projects and tasks from external MCP clients like Claude Desktop, Raycast, or even coding agents running within Crew itself.

<Note>
This page covers connecting **external MCP clients** to Crew's MCP server. For configuring MCP servers **within** Crew for your coding agents, see the [MCP Server Configuration](/integrations/mcp-server-configuration) guide.
</Note>

<Info>
Crew's MCP server is **local-only** - it runs on your computer and can only be accessed by applications installed locally. It cannot be accessed via publicly accessible URLs.
</Info>
...
This works if you're adding the Crew MCP server to any [supported coding agent](/supported-coding-agents) **within** Crew.

1. In Crew Settings, navigate to the "MCP Servers" page
2. In the "Popular servers" section, click on the Crew card
...
The Crew MCP server provides the following tools for managing projects, tasks, and task execution:
...
3. **Automatic Task Generation**: Your MCP client will use the Crew MCP server to automatically create structured tasks in your project
...
Your MCP client will use the `create_task` tool to break this down into individual tasks and add them to your Crew project automatically.
...
### Internal Coding Agents (Within Crew)

A powerful workflow involves using coding agents within Crew that are also connected to the Crew MCP server:
...
4. **Automatic Population**: The agent uses the MCP server to populate individual tasks directly in the Crew interface
...
Raycast is a popular MCP client that can connect to Crew's MCP server. Here's how to configure it:
